...
function getBuddyListFriends(list: BuddyList): Friend[] {
  //reduce callback : 
  // check each buddy. If it has a phone property
  //then it's a friend, so add it to the friends arr
  // acc - > accumulator
  // buddy -> current item
  return list.members.reduce<Friend[]>((acc, buddy) => {
    ...
    //Type guard here
    if ("phone" in buddy){
      acc.push(buddy) // Update the accumulator
    }
    return acc; // reduce needs to return the accumulator
  }, []); // intial value
}

const friendsListCheck = getBuddyListFriends(myFootballBuddies)
console.log("Friends extracted:",friendsListCheck)